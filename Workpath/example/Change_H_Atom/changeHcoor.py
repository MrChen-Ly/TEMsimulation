...
# 替换并保存文件
def replace_and_save_files(coordinates1, input_file, output_prefix):
    idx = 1
    num_digits = len(str(len(coordinates1)+1))
    print(num_digits)
    for x1, y1, z1 in coordinates1:
        # 替换内容，使用两组坐标
        #new_line_12 = f" H    {x1:.6f}  {y1:.6f}  0.330095  1.000000  0.003000  0.100000  0.100000  0.100000"
        #new_line_14 = f" H    {x1:.6f}  {(y1+0.5):.6f}  0.330095  1.000000  0.003000  0.100000  0.100000  0.100000"
...
def process_coordinates_for_two_points(coords1, input_file):
    x0_1, y0_1, z0_1 = coords1

    # 自定义范围和步长（可以根据需要调整）
    x_change = [[0.01, 0.04]]  # [步长, 范围]
    y_change = [[0.01, 0.08]]
    z_change = [[0.01, 0.00]]

    # 生成两组坐标
    coordinates1 = generate_coordinates(x0_1, y0_1, z0_1, x_change, y_change)


    # 替换并保存文件
    replace_and_save_files(coordinates1,  input_file, "STO_110_Output")
...
# 示例输入
input_file = 'STO_110_Output_073.cel'
coords1 = (0.297500, 0.461300, 0.330095)


# 执行程序
process_coordinates_for_two_points(coords1, input_file)
